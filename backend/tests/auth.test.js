...
const mockValidToken = () =>
  admin.auth().verifyIdToken.mockResolvedValue(FAKE_USER);
...
describe("Auth API", () => {
  ...
  describe("POST /api/auth/sync", () => {
    it("syncs a new Firebase user to MySQL and creates a Redis session", async () => {
      mockValidToken();
      db.query
        .mockResolvedValueOnce([[]]) // check if existing
  ...
  describe("POST /api/auth/logout", () => {
    it("revokes Firebase tokens and blacklists the session", async () => {
      ...
      expect(redis.set).toHaveBeenCalledWith(
        "session_blacklist:firebase-uid-123",
        "1",
        "EX",
        expect.any(Number),
      );
  ...
  describe("POST /api/auth/forgot-password", () => {
    it("generates a reset link and sends it via email", async () => {
      admin
        .auth()
        .generatePasswordResetLink.mockResolvedValue("https://reset.link/test");
